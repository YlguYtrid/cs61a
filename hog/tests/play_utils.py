...
def describe_game(hog, test_number, score0, score1, goal, update):
    ...
    s0last, s1last, game_trace = trace_play(hog.play, strategy0, strategy1, update=update, score0=score0, score1=score1, dice=dice, goal=goal, say=None)
    ...
    for turn, end in zip(game_trace, end_of_turn):
        summary.append(SUMMARY.format(s0=turn['s0_start'], s1=turn['s1_start'], w=turn['who'], nr=turn['num_dice'], rv=turn['dice_values'], e0=end[0], e1=end[1]))
